...
class PacketGenerator(object):
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            self.out.put(p)
            logger.debug(f"Packet generated in {self.__class__.__name__} {self.id} at {self.env.now}: " + str(p))
...
class PacketSink(object):
    ...
    """
    def __init__(self, env, rec_arrivals=False, absolute_arrivals=False, rec_waits=True, debug=False, selector=None, id=None):
    ...
    def put(self, pkt):
        if not self.selector or self.selector(pkt):
            ...
            logger.debug(f"Pakcet received in {self.__class__.__name__} {self.id} at {self.env.now}: " + str(pkt) )
            if self.debug:
                print(pkt)
...
class SwitchPort(object):
    ...
    def put(self, pkt):
        self.packets_rec += 1
        logger.debug(f"Packet received in {self.__class__.__name__} {self.id} at {self.env.now}: " + str(pkt))
        ...
        if self.limit_bytes and tmp_byte_count >= self.qlimit:
            logger.debug(f"Packet dropped in {self.__class__.__name__} {self.id} at {self.env.now}: " + str(pkt))
            self.packets_drop += 1
            return
